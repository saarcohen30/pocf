...
def run_single_experiment(n: int, M: int, policy: str, seed: int,
    ...
    print(f"Running experiment n={n}, M={M}, k={K}, policy={policy}, seed={seed} ...")
...
def run_experiment_grid_for_model(utility_model: str,
    ...
    if impact == 'n':
        ...
        for n in n_list:
            for M in M_values:
                all_tasks.append(('A', n, M, K_for_n_plot))
    else: # Family B (n fixed to 25)
        n_fixed = 25
        all_tasks = []
        for K in K_list_for_Kplot:
            for M in M_values:
                all_tasks.append(('B', n_fixed, M, K))
...
# -----------------------
def run_all_models(policy: str, parallel_jobs: int = 1):
    ...
    for utility_model in UTILITY_MODELS:
        ...
        results_A, results_B = run_experiment_grid_for_model(utility_model=utility_model,
                                                            policy=policy,  # you can change to 'oneRand' if desired
                                                            n_list=n_list,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", type=str, default='random', help="Exploration policy")
    ...
    parser.add_argument("--impact", type=str, default='n', choices=['n', 'k'], help="Impact of the number of agents n or the number of candidate coalitions k.")
    ...
    if args.single_model is not None:
        ...
        start = time.time()
        results = run_experiment_grid_for_model(utility_model=model,
                                                            policy=policy,
                                                            n_list=[5, 10, 15, 20, 25],
                                                            K_for_n_plot=5,
                                                            K_list_for_Kplot=[5,10,15,20,25],
        ...
        if impact == 'n':
            plot_impact_of_n(results, [5, 10, 15, 20, 25], [100, 5000, 10000, 20000, 30000], model, outname=f"impact_n_{model}_{policy}.png", policy=policy)
        else:
            plot_impact_of_K(results, [5,10,15,20,25], [100, 5000, 10000, 20000, 30000], model, outname=f"impact_K_n25_{model}_{policy}.png", policy=policy)
            
        with open(os.path.join(OUT_DIR, f"raw_results_{model}_.pkl"), "wb") as f:
            pickle.dump({'results': results}, f)
        print("Done. Time:", time.time() - start)
    else:
        run_all_models(policy=policy, parallel_jobs=args.parallel)